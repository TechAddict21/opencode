...
import { makeRuntime } from "@nous-ai/core/effect/runtime"
...
import type { TuiAttentionSoundName } from "@nous-ai/plugin/tui"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // Every config dir we may read from: global config dir, any `.noussh`
  ...
  // 4. `.noussh` directories (and OPENCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".noussh") || dir === Flag.OPENCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".noussh") && dir !== Flag.OPENCODE_CONFIG_DIR) continue
  ...
  const result: Resolved = {
    ...
    attention: {
      ...
      sound_pack: acc.result.attention?.sound_pack ?? "nous.default",

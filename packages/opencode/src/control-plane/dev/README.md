This is a plugin to simulate a remote environment locally. Add this to `.noussh/nous.jsonc`:

```json
  "plugin": ["../packages/nous/src/control-plane/dev/debug-workspace-plugin.ts"],
```
...
```
./packages/nous/script/run-workspace-server
```

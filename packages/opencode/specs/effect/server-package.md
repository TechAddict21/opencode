...
Practical reference for a future `packages/server` split after the nous
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/nous`.
...
Until enough shared service code lives outside `packages/nous`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/nous`

It should not import `packages/nous` services while `packages/nous`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on nous-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/nous` implementation details.

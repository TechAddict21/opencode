...
import nous from "./theme/nous.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@nous-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "nous")
        draft.active = typeof active === "string" ? active : "nous"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.nous, store.mode)
    })

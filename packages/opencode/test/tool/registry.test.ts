...
describe("tool.registry", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const opencode = path.join(test.directory, ".noussh")
        const customTools = path.join(opencode, "tools")
        const plugin = path.join(opencode, "node_modules", "@nous-ai", "plugin")
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@nous-ai/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@nous-ai/plugin"',
              "export default tool({",

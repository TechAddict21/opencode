...
Speed up the `packages/nous` test suite without reducing coverage or hiding failures.

## Benchmark Command

Run from `packages/nous`:
...
`packages/nous/test/**`, test fixtures, package test scripts, and implementation setup paths only when a benchmarked bottleneck points there.
...
| HTTP provider tests generate local plugins without dependency-ready fixture state                         | Marked generated `.nous` plugin fixtures dependency-ready                                  | 7.905s    | 2.980s  | keep     | Median from 3 targeted runs; avoids unrelated plugin dependency setup in route tests.                                            |
...
| `.nous` agent and command file-loading cases can use Effect-aware instance fixtures                   | Migrated singular/plural agent and command markdown fixture cases to `it.instance`             | 7.21s     | 1.87s   | keep     | Parent baseline was noisy (7.42, 7.21, 2.83); after runs were stable at 1.87, 1.98, 1.83. Keep as cleanup with no broad claim.   |

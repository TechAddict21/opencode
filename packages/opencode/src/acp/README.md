...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for nous.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal nous sessions
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 nous acp
```
...
- **Terminal Integration**: Full terminal support via nous's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | nous acp
```
...
ACP sessions map cleanly to nous's internal session model:

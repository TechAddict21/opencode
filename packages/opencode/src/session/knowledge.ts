...
// no disk reads here), bounded by the injection budget.
function assembleDocs(index: KBIndex, matched: string[]): string {
  ...
  for (const ep of matched) {
    if (total >= KB.MAX_INJECTION_BYTES) break
    const entry = byPath.get(ep)
    if (!entry || !entry.docText.trim()) continue
    const header = `\n### ${ep}\n`
    const remaining = KB.MAX_INJECTION_BYTES - total - header.length
    if (remaining <= 0) break
    const body =
      entry.docText.length > remaining
        ? entry.docText.slice(0, remaining) + "\n... [truncated — read the source files for more]"
        : entry.docText
    parts.push(header + body)
    total += body.length + header.length
  }
  return parts.join("\n")
}
...
2. Files explored this session that are NOT yet covered by the knowledge base.
3. The current DRILL_DOWN_TREE.md.
...
- Put EVERY source-file path inside the DOC's "## Key Files" section, one per line as: \`path/to/file.ext\` — its use case.
- Prefer TRUE domains. If the explored files fit an existing area, REUSE that area's exact name and doc path and extend its doc (add the new files to Key Files) rather than creating a new, overlapping area.
- Record only knowledge actually demonstrated this session. Do NOT guess or invent files, endpoints, or behaviour.
  ...
  Effect.gen(function* () {
    ...
    const buildIndex = (workDir: string, treeContent: string) =>
      Effect.gen(function* () {
        ...
        const index: KBIndex = { treeContent, entries, pathToEntries, baseToEntries, dirToEntries }
        return index
      })
    ...
    const feeder = Effect.fn("Knowledge.feeder")(function* (userText: string, userMessageID: string) {
      ...
      if (treeContent && treeContent.trim()) {
        ...
        if (index) {
          ...
          // Try harder: if a conceptual query (no explicit file path) matched no
          // area, surface the KB index so the agent can navigate itself instead
          // of concluding the KB has nothing.
          if (!docs && extractQueryPaths(userText).length === 0) {
            indexHint = assembleIndexHint(index)
          }
          log.info("feeder matched", { entries: matchedEntries.length, matchedEntries, indexHint: indexHint.length > 0 })
        }
      }
    ...
    const completerCore = Effect.fn("Knowledge.completer")(function* (sessionID: string, messages: MessageV2.WithParts[]) {
      ...
      yield* Effect.gen(function* () {
        ...
        if (docBody.length > KB.MAX_DOC_BYTES) docBody = docBody.slice(0, KB.MAX_DOC_BYTES)
        ...
        yield* fs.writeFileString(fullPath, docBody).pipe(Effect.orElseSucceed(() => {}))
        log.info("completer wrote doc", { docRel })

        const bodyLines = `- **${docRel}** — ${summary}`
        const newTree = KB.upsertAreaSection(treeContent, areaName, bodyLines)
        if (newTree.trim() !== treeContent.trim()) {
          yield* fs

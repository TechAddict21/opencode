...
- Author package entrypoint is `@nous-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/nous-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@nous-ai/plugin/tui`.
- `@nous-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@nous-ai/plugin/tui"
...
- If `engines.nous` is absent, no check is performed (backward compatible).
...
- `nous plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `nous plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.nous` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.nous`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.nous`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.nous` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@nous-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@nous-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@nous-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"nous"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `nous.default`.
...
- Local plugins persist installed themes under the local `.nous/themes` area near the plugin config source.
...
- Local smoke plugin: `.nous/plugins/tui-smoke.tsx`
- Local vim plugin: `.nous/plugins/tui-vim.tsx`
- Local smoke config: `.nous/tui.json`
- Local smoke theme: `.nous/plugins/smoke-theme.json`

...
Replace runtime Bun shell template-tag usage in `packages/nous/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/nous/src` is removed except:

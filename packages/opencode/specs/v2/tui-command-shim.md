...
Delete `packages/nous/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/nous/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/nous/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/nous`
- TUI plugin loader tests in `packages/nous` if runtime plugin API wiring changed
